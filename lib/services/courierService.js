  ...
  EsriService.getToken(function(token){
    ...
    request(options, function(error, response, body){
      if(error) console.log('error', error);
        ...
        console.log(json);
        if(json !== undefined){
          if(json.directions !== undefined){
          var time = json.directions[0].summary.totalTime;
          callback(null, time, courier, order, callback2);
          }
          else callback('json.directions is undefined', null, null, null, callback2);
        } else {
          callback('oeps er is iets misgegaan', null, null, null, callback2);
        }
      }
...
var fastestCourier = function(error, time, courier, order, callback){
