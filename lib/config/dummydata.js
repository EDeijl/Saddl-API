'use strict';

var mongoose = require('mongoose'),
  User = mongoose.model('User'),
  Thing = mongoose.model('Thing'),
  Courier = mongoose.model('Courier'),
  CourierStatus = mongoose.model('CourierStatus'),
  Order = mongoose.model('Order'),
  OrderStatus = mongoose.model('OrderStatus'),
  Package = mongoose.model('Package'),
  Route = mongoose.model('Route'),
  Specialty = mongoose.model('Specialty'),
  Transport = mongoose.model('Transport'),
  TransportType = mongoose.model('TransportType');

/**
 * Populate database with sample application data
 */

//Clear old things, then add things in
Thing.find({}).remove(function() {
  Thing.create({
    name : 'HTML5 Boilerplate',
    info : 'HTML5 Boilerplate is a professional front-end template for building fast, robust, and adaptable web apps or sites.',
    awesomeness: 10
  }, {
    name : 'AngularJS',
    info : 'AngularJS is a toolset for building the framework most suited to your application development.',
    awesomeness: 10
  }, {
    name : 'Karma',
    info : 'Spectacular Test Runner for JavaScript.',
    awesomeness: 10
  }, {
    name : 'Express',
    info : 'Flexible and minimalist web application framework for node.js.',
    awesomeness: 10
  }, {
    name : 'MongoDB + Mongoose',
    info : 'An excellent document database. Combined with Mongoose to simplify adding validation and business logic.',
    awesomeness: 10
  }, function() {
      console.log('finished populating things');
    }
  );
});

var user1 = new User({_id: new mongoose.Types.ObjectId(), provider: 'local', name: 'Test User', email: 'test@test.com', password: 'test'});
// Clear old users, then add a default user
User.find({}).remove(function() {
  User.create(user1
  , function() {
      console.log('finished populating users');
    }
  );
});
var courier1 = new Courier({_id : new mongoose.Types.ObjectId(), name: 'Henk', picture: 'http://placehold.it/350x350'});
var courier2 = new Courier({_id : new mongoose.Types.ObjectId(), name: 'Harry', picture: 'http://placehold.it/350x350'});
var courier3 = new Courier({_id : new mongoose.Types.ObjectId(), name: 'Harrold', picture: 'http://placehold.it/350x350'});
var courier4 = new Courier({_id : new mongoose.Types.ObjectId(), name: 'Henk', picture: 'http://placehold.it/350x350'});

var package1 = new Package({_id : new mongoose.Types.ObjectId(), length: 10, width: 10, height: 10, weight: 10});
var package2 = new Package({_id : new mongoose.Types.ObjectId(), length: 10, width: 10, height: 10, weight: 10});
var package3 = new Package({_id : new mongoose.Types.ObjectId(), length: 10, width: 10, height: 10, weight: 10});
var package4 = new Package({_id : new mongoose.Types.ObjectId(), length: 10, width: 10, height: 10, weight: 10});

var order1 = new Order({origin: 'Rotterdam', destination: 'Spijkenisse', _packages: [package1._id, package2._id, package3._id], _courier: courier1._id, _user: user1._id});
var order2 = new Order({origin: 'Rotterdam', destination: 'Spijkenisse', _packages: [package1._id, package2._id, package3._id], _courier: courier2._id});
var order3 = new Order({origin: 'Rotterdam', destination: 'Spijkenisse', _packages: [package1._id, package2._id, package3._id], _courier: courier3._id});
Courier.find({}).remove(function() {
  Courier.create(courier1, courier2, courier3, courier4, function() {
    console.log('finished populating couriers');
  });
});
Package.find({}).remove(function() {
  Package.create(package1, package2, package3, package4, function() {
    console.log('finished populating packages');
  });
});

Order.find({}).remove(function () {
  Order.create(order1, order2, order3,function(){
    console.log('finished populationg orders');
  } );
});
